...
float mynoise2( vec2 x )
{
    ...
    return mix(s1.x,s1.y,f.y)*1.0;
}

float mynoise3( vec3 x )
{
    vec3 p = floor(x);
    vec3 f = fract(x);
    f = f*f*(3.0-2.0*f);
    ...
    return mix(mix(s1.x,s1.y,f.y),mix(s1.z,s1.w,f.y),f.z)*1.0;
}
...
vec4 booble(vec2 te,vec2 pos,float numCells)
{
    ...
    vec2 te2=-te1;
    float zb1=max(pow(mynoise2(te2*1000.11*d),10.0),0.01);
    float zb2=mynoise2(te1*1000.11*d);
    float zb3=mynoise2(te1*200.11*d);
    float zb4=mynoise2(te1*200.11*d+vec2(20.0));
    
    vec4 colorb=vec4(1.0);
    colorb.xyz=colorb.xyz*(0.7+mynoise2(te1*1000.11*d)*0.3);
    
    zb2=max(pow(zb2,20.1),0.01);
    colorb.xyz=colorb.xyz*(zb2*1.9);
    
    vec4 color=vec4(mynoise2(te2*10.8),mynoise2(te2*9.5+vec2(15.0,15.0)),mynoise2(te2*11.2+vec2(12.0,12.0)),1.0);
    color=mix(color,vec4(1.0),mynoise2(te2*20.5+vec2(200.0,200.0)));
    color.xyz=color.xyz*(0.7+mynoise2(te2*1000.11*d)*0.3);
...
vec4 Cells(vec2 p,vec2 move, in float numCells,in float count,float blur)
{
    ...
    //te=te+(te*mynoise3(vec3(te*5.9,iTime*40.0))*0.02);
    //te=te+(te*(mynoise3(vec3(te*3.9+p,iTime*0.2))+mynoise3(vec3(te*3.9+p,iTime*0.2+0.25))+mynoise3(vec3(te*3.9+p,iTime*0.2+0.5))+mynoise3(vec3(te*3.9+p,iTime*0.2+0.75)))*0.05);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //vec4 e=vec4(mynoise2(uv*2.0),mynoise2(uv*2.0+vec2(200.0)),mynoise2(uv*2.0+vec2(50.0)),0.0);
    vec4 e=vec4(mynoise3(vec3(uv*2.0,iTime*0.1)),mynoise3(vec3(uv*2.0+vec2(200.0),iTime*0.1)),mynoise3(vec3(uv*2.0+vec2(50.0),iTime*0.1)),0.0);
    e.rgb = vec3(0.0);
